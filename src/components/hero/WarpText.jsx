...
const measureLine = (ctx, line, letterSpacing) => {
  ...
  const textWidth = chars.reduce((w, char) => w + ctx.measureText(char).width, 0);
  return textWidth + Math.max(0, chars.length - 1) * letterSpacing;
};

const drawLine = (ctx, line, centerX, y, letterSpacing) => {
  if (!letterSpacing) {
  ...
  ctx.textAlign = 'left';
  const chars = Array.from(line);
  let cursor = centerX - measureLine(ctx, line, letterSpacing) / 2;
...
const buildTextCanvas = ({ width, height, dpr, props, fallbackEl }) => {
  ...
  let fontSizePx = 96;
  let fontFamily = "'Big Shoulders Display', system-ui, sans-serif";
  let fontWeight = String(props.fontWeight || '800');
  let letterSpacing = 0;
  let lineHeight = 88;

  if (fallbackEl) {
    const computed = window.getComputedStyle(fallbackEl);
    ...
    lineHeight = Number.isFinite(computedLineHeight) ? computedLineHeight : fontSizePx * (props.lineHeight || 0.92);
  } else {
    if (typeof props.fontSize === 'number') fontSizePx = props.fontSize;
    lineHeight = fontSizePx * (props.lineHeight || 0.92);
  ...
  const setFont = (size) => {
    ctx.font = `${fontWeight} ${Math.round(size)}px ${cleanFontFamily}`;
  };
...
const WarpText = ({
  ...
  useEffect(() => {
    ...
    if (!gl) {
      console.warn('WarpText: No WebGL context available.');
      return undefined;
    }
    ...
    const isV2 = renderer.isWebgl2;

    try {
      program = new Program(gl, {
        vertex: isV2 ? vertex300 : vertex100,
        fragment: isV2 ? fragment300 : fragment100,
        transparent: true,
        depthTest: false,
        depthWrite: false,
        uniforms: {
      ...
      console.warn('WarpText: Program compilation failed.', error);
      return undefined;
    }
    ...
    const drawToTexture = () => {
      ...
      const textCanvas = buildTextCanvas({
        ...
        fallbackEl: fallbackRef.current
      });
