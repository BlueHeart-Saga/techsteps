...
async function run() {
  try {
    ...
    console.log('\n--- 1. Querying Kudu Root API ---');
    const apiRes = await kuduRequest('GET', '/api');
    console.log('GET /api response snippet:\n' + apiRes.data.slice(0, 500));

    console.log('\n--- 2. Inspecting /home/LogFiles via bash ---');
    const logFilesRes = await kuduRequest('POST', '/api/command', JSON.stringify({
      command: 'bash -c "ls -la /home/LogFiles && ls -la /home/LogFiles/docker 2>/dev/null || true"',
      dir: '/home/site/wwwroot',
    }));
    console.log('LogFiles:\n' + logFilesRes.data);

    const dockerLogRes = await kuduRequest('POST', '/api/command', JSON.stringify({
      command: 'bash -c "cat /home/LogFiles/*_docker.log 2>/dev/null | tail -n 100 || true"',
      dir: '/home/site/wwwroot',
    }));
    console.log('Docker Log Tail:\n' + dockerLogRes.data);

    const appDockerLogRes = await kuduRequest('POST', '/api/command', JSON.stringify({
      command: 'bash -c "cat /home/LogFiles/docker/* 2>/dev/null | tail -n 100 || true"',
      dir: '/home/site/wwwroot',
    }));
    console.log('Docker App Logs:\n' + appDockerLogRes.data);

    console.log('\n--- 3. Checking site/diagnostics and startup scripts ---');
    const startupFindRes = await kuduRequest('POST', '/api/command', JSON.stringify({
      command: 'bash -c "find /home -maxdepth 3 -name \"*startup*\" -o -name \"*.sh\" 2>/dev/null || true"',
      dir: '/home/site/wwwroot',
    }));
    console.log('Found startup files:\n' + startupFindRes.data);
